 ...
 * used instead.
 */
public class NonArRenderer implements GLSurfaceView.Renderer {
	/** Used for debug logs. */
	private static final String TAG = "nonArRenderer";

	private final NonArView Non_ar_view;
	 ...
	 */
	public NonArRenderer(final NonArView ArActivity, final GLSurfaceView glSurfaceView) {
		Non_ar_view = ArActivity;
				...
				mGlSurfaceView.queueEvent(new Runnable() {
					...
					public void run() {												
						...
						try {
							...
							Non_ar_view.runOnUiThread(new Runnable() {
								@Override
								public void run() {
								}
							});										
	...
	{		
		...
		final String vertexShader = RawResourceReader.readTextFileFromRawResource(Non_ar_view, R.raw.non_ar_vertex_shader);
 		final String fragmentShader = RawResourceReader.readTextFileFromRawResource(Non_ar_view, R.raw.non_ar_fragment_shader);
		...
		// Load the texture
		mAndroidDataHandle = TextureHelper.loadTexture(Non_ar_view, R.drawable.key);
